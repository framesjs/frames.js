...
import { parallelizeMiddleware } from "./parallelizeMiddleware";

describe("parallelizeMiddleware", () => {
  it("throws if no middleware is provided", () => {
    expect(() => parallelizeMiddleware()).toThrow("No middlewares provided");
  });

  it("returns middleware as is if only one is provided", () => {
    const middleware = async () => {};

    expect(parallelizeMiddleware(middleware as any)).toBe(middleware);
  });
  ...
  it("returns a middleware that wraps all provided middlewares", () => {
    ...
    const middleware2 = async (contexxt: any, next: any) => {
      return next();
    };

    const parallelMiddleware = parallelizeMiddleware(middleware1, middleware2);
