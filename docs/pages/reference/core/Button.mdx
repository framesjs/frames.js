...
Tx button requests transaction or signature data from `target` when clicked, and sends the transaction ID or signature hash to the frame's `post_url` or the button's `post_url` if it is set.
