...
A string that specifies the base path for all relative URLs in the frame definition. It defaults to `/`. If `baseUrl` is provided, it will be resolved relatively to `baseUrl`. If the `baseUrl` option is not provided, it will overwrite the path of current request's URL.
...
Specifies the base URL for all relative URLs in the frame definition. `baseUrl` is resolved relatively to this value.
