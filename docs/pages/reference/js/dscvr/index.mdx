...
First, you need to install `frames.js` and `@dscvr-one/frames-adapter`. You can do this by running the following command:

<HomePage.InstallPackage
  name="frames.js @dscvr-one/frames-adapter"
  type="install"
/>
...
} from 'frames.js/dscvr'; // [!code focus]
```
...
if (
  ...
  // [!code focus]
  const frameMessage = await getDscvrFrameMessage(previousFrame.postBody); // [!code focus]
...
if (
  ...
  const frameMessage = await getDscvrFrameMessage(previousFrame.postBody);
