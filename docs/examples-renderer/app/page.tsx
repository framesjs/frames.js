...
export default function Home({
  ...
  const farcasterIdentity = useFarcasterIdentity({
    onMissingIdentity() {
      // eslint-disable-next-line no-console -- logging for the developer
      console.log("select an identity");
    },
  });
  ...
  useEffect(() => {
    farcasterIdentityRef.current.impersonateUser(1).catch((e) => {
      // eslint-disable-next-line no-console -- logging for the developer
      console.error(e);
    });
  }, []);
  ...
  return (
    <main
      className={`p-2 ${frameState.frame?.status === "pending" ? "blur" : ""} min-w-[500px] min-h-[336px]`}
    >
